...
# Restrict the admin panel (UI + /api) to networks, e.g. your VPN egress.
# Proxy routes (/_slug) and static sites stay public. Comma-separated IPv4
# CIDRs/IPs or hostnames (resolved via DNS, auto-refreshed). Empty/unset = open.
# Restart the service after changing.
#HORMUZ_PANEL_ALLOW_CIDRS=vpn.example.com
EOF
  chmod 600 "$ENV_FILE"
else
